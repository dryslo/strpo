...
int main()
{
    ...
    {
        Rect rect; // область памяти выделяется, значения не определены(случайный шум)
    }
     ...
     * конструктора. // из за создания конструктора, конструктор по умолчанию перестал существовать
     *
    ...
    {
        Rect rect1(1, 2, 3, 4);
        Rect rect2 = rect1; // вызывается конструктор копирования, которые определен по умолчанию(несмотря на знак =, это инициализация, а не присваивание)
        Rect rect3(rect1); // по умолчанию объект будет скопирован побайтово
    }
     ...
     * случае? // деструктор вызывается при удалении объекта
     * // по умолчанию он определен
     *
...

    {
        ...
        Rect *pR = new Rect(1,2,1,2); // конструктор с парметрами
        {
            Rect r2(r1); // конструктор копирования
            Rect arRect[2]; // 2 обычных конструктора
            for(int i = 0; i < 3; i++)
            {
                static Rect r3 (i,i,i,i) ; // только 1 раз констуктор с параметрами
                Rect r4(*pR); // конструктор копирования
                Rect r5(i,i,i,i); // конструктор с параметрами
                // деструктор r5
                // деструктор r4
            }
            //деструктор для каждого arRect 
        }
        delete pR; // деструктор pR
        //деструктор r1
        // в конце выполнения программы деструктор r3 
    }
    ...
    {
        Rect r1;
        //std::cout << r1.x1 << std::endl; // приватные поля и методы недоступны вне класса
    }
     ...
     */
    // функция член находится внутри класса, имеет доступ к его функциям, значениям, неявно передает this
    ...
    {
        Rect r1(1,2,5,4), r2(2,2,4,4); // конструкторы с параметрами
        Rect r3 = bounding_rect(r1, r2); // конструктор с параметрами внутри bounding_rect
     ...
     * `get_suare()`, возвращающие ширину, высоту и площадь соответственно,
    ...
    } */
   {
    ...
    Barrel alch(V,0.96);
    Barrel water(V,0);
    while(alch.get_conc() > 0.5){
        alch.fill_to_other(water,1);
        water.fill_to_other(alch,1);
        i++;
    }
    std::cout<<i<<std::endl;
   }
...

    {
        MyString s1;
        s1.print();
        MyString s2("My string eee");
        s2.print();
        std::cout<<s2.get(3)<<std::endl;
        s2.set(3,'S');
        s2.print();
        s2.set_new_string("My new string eee");
        s2.print();
        s2.read_line();
        s2.print();
    }
